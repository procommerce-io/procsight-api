...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\ProCurrency\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/ProCurrency/';
  if (isLinux) dataDir = process.env.HOME + '/.procurrency/';
}
...
var bitcoindConf = {
  ...
  user: process.env.BITCOIND_USER || 'procrpc',
  pass: process.env.BITCOIND_PASS || 'procpass',
